        ...
        $.fn.smartZoom = function(method) {
            ...
            var publicMethods = {
                ...
                zoom: function(scaleToAdd, globalRequestedPosition, duration) {
                    ...
                    //if(newScale.toFixed(2)>=1) newScale = 1;
            ...
            function updateMouseMoveCursor() {
                ...
                if (smartData.settings.dblClickMaxScale <= (currentScale + 0.05))
                    targetElement.addClass("out");
                else
                    targetElement.removeClass("out");
            }
            ...
            function adjustToContainer() {
                ...
                var w = containerDiv.parent().width() < getContainerDivSizeFromSettings(smartData.settings.width, containerDiv.parent().width(), containerDivNewLeft - parentOffset.left);
                w = true ? containerDiv.parent().width() : getContainerDivSizeFromSettings(smartData.settings.width, containerDiv.parent().width(), containerDivNewLeft - parentOffset.left);
                var h = containerDiv.parent().height() < getContainerDivSizeFromSettings(smartData.settings.height, containerDiv.parent().height(), containerDivNewTop - parentOffset.top);
                h = true ? containerDiv.parent().height() : getContainerDivSizeFromSettings(smartData.settings.height, containerDiv.parent().height(), containerDivNewTop - parentOffset.top);
                containerDiv.width(w); // apply size found to zoomablecontainer 
                containerDiv.height(h);
            ...
            function windowResizeEventHandler() {
                clearTimeout(doit);
                doit = setTimeout(adjustToContainer, 100);
            }
